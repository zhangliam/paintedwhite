...
function execute(node, scope) {
  ...
  switch (type) {
    ...
    case 'BooleanLiteral':
      return !!node.value;
    case 'Identifier':
      return scope[node.value] != undefined ? scope[node.value] : '';
    case 'CallExpression':
    ...
    case '.':
      return execute(children[0], scope)[children[1].value]
    case '[':
      return execute(children[0], scope)[toNumber(execute(children[1], scope))]
